...
def load_model(model_path, args):
    ...
    model.cuda()
    model.eval()
    #state_dict = torch.load(model_path, map_location='cpu')  # <-----------修改加载模型的路径
...
def box2pos(file,boxes,resolution,w,h):
    for i in range(len(boxes)):
        ...
        if file.split('_')[1][0].isalpha():
            ...
            with open(pos_save_path + "\\_output_inter.txt", 'a+') as f:
            ...
            start1 = int((100 * 0.8) * (row - 1) + (ymin / (h/100))) * resolution
            end1 = int((100 * 0.8) * (row - 1) + (ymax / (h/100))) * resolution
            start2 = int((100 * 0.8) * (col - 1) + (xmin / (w/100))) * resolution
            end2 = int((100 * 0.8) * (col - 1) + (xmax / (w/100))) * resolution
            with open(pos_save_path + "\\_output_intra.txt", 'a+') as f:
...
if __name__ == "__main__":
    ...
    main_args.use_feature_fusion = True  
    main_args.backbone = 'resnet50'  # 与训练时一致
    # 加载模型
    dfdetr = load_model('/mnt/sdc/wanghaojie/newstart/checkpoint.pth', main_args)  # <--修改为自己加载模型的路径
    resolution = 50000
    img_load_path = "/mnt/sdc/wanghaojie/HIC/K562_intra_image_50000"
    files = os.listdir(img_load_path)  # <--修改为待预测图片所在文件夹路径
    img_save_path = f"/mnt/sdc/wanghaojie/newstart/K562/K562_intra_result"
    pos_save_path = os.path.join(img_save_path,"labels")
